...
#define LH_SYSTEM_VERSION_LESS_THAN(v) ([[[UIDevice currentDevice] systemVersion] compare:v options:NSNumericSearch] == NSOrderedAscending)
#define LH_IS_IPAD   (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
...
@interface LHPopupController () <UIGestureRecognizerDelegate>
...
@implementation LHPopupController
...
- (instancetype)initWithContents:(NSArray <UIView *> *)contents {
    ...
    if (self) {
        ...
        self.theme = [LHPopupTheme defaultTheme];
...
- (void)orientationChanged {
    ...
    CGFloat angle = LH_UIInterfaceOrientationAngleOfOrientation(statusBarOrientation);
    CGAffineTransform transform = CGAffineTransformMakeRotation(angle);
    
    [UIView animateWithDuration:self.theme.animationDuration animations:^{
        self.maskView.frame = self.applicationWindow.bounds;
        self.popupView.center = [self endingPoint];
        if (LH_SYSTEM_VERSION_LESS_THAN(@"8.0")) {
            self.popupView.transform = transform;
        }
    }];
}

CGFloat LH_UIInterfaceOrientationAngleOfOrientation(UIInterfaceOrientation orientation)
{
    CGFloat angle;
...
- (void)applyTheme {
    if (self.theme.popupStyle == LHPopupStyleFullscreen) {
        self.theme.presentationStyle = LHPopupPresentationStyleFadeIn;
    }
    if (self.theme.popupStyle == LHPopupStyleActionSheet) {
        self.theme.presentationStyle = LHPopupPresentationStyleSlideInFromBottom;
    }
    self.popupView.layer.cornerRadius = self.theme.popupStyle == LHPopupStyleCentered?self.theme.cornerRadius:0;
    ...
    if (self.theme.popupStyle == LHPopupStyleFullscreen) {
        ...
        maskBackgroundColor = self.theme.maskType == LHPopupMaskTypeClear?[UIColor clearColor] : [UIColor colorWithWhite:0.0 alpha:0.7];
    }
...
- (CGFloat)popupWidth {
    ...
    if (width > maskViewWidth || self.theme.popupStyle == LHPopupStyleFullscreen) {
        width = maskViewWidth;
    }
    return width;
}
...
#pragma mark - LHPopupButton Methods

@implementation LHPopupButton
...
#pragma mark - LHPopupTheme Methods

@implementation LHPopupTheme

+ (LHPopupTheme *)defaultTheme {
    LHPopupTheme *defaultTheme = [[LHPopupTheme alloc] init];
    defaultTheme.backgroundColor = [UIColor whiteColor];
    defaultTheme.cornerRadius = 4.0f;
    defaultTheme.popupContentInsets = UIEdgeInsetsMake(0, 0, 0, 0);
    defaultTheme.popupStyle = LHPopupStyleCentered;
    defaultTheme.presentationStyle = LHPopupPresentationStyleSlideInFromTop;
    defaultTheme.dismissesOppositeDirection = YES;
    defaultTheme.maskType = LHPopupMaskTypeDimmed;
