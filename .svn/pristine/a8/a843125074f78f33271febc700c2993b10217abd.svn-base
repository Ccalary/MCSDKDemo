...
@property (nonatomic, assign) BOOL isShowEmoji; //是否显示表情切换按钮
...
- (id)initWithFrame:(CGRect)frame andIsShowEmoji:(BOOL)isShow{
    self = [super initWithFrame:frame];
    if (self) {
        self.isShowEmoji = isShow;
        self.inputContainerSubViewConstraints = [[NSMutableArray alloc] init];
        [self resetInputBar];
        self.currentPositionY = frame.origin.y;
        self.originalPositionY = frame.origin.y;
...
- (void)setLayoutForInputContainerView {
    ...
    NSString *format;
    if (self.isShowEmoji){
        format =  @"H:|-0-[_switchButton(0)]-10-[_inputTextView]-10-[_emojiButton(30)" @"]-10-|";
    }else {
        format =  @"H:|-0-[_switchButton(0)]-10-[_inputTextView]-10-[_emojiButton(0)" @"]-0-|";
    }
...
- (void)didTouchEmojiDown:(UIButton *)button{
    if (self.inputTextView.hidden) {
        [self switchInputBoxOrRecord];
    }
    [self.delegate didTouchEmojiButton:button];
}
